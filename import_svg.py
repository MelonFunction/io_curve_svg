...
def SVGGetMaterial(color, context, opacity):
    """
    ...
    mat.diffuse_color = (*diffuse_color, float(opacity))
...
def SVGParseStyles(node, context):
    ...
    if style:
        ...
        if styles['useFill']:
            styles['material'] = SVGGetMaterial(styles['fill'], context, styles['fill-opacity'])
        else:
            styles['useFill'] = True
            styles['material'] = SVGGetMaterial('#000', context, 1.0)
    ...
    if styles['useFill'] is None:
        ...
        if fill:
            ...
            if fill == 'none':
                ...
                styles['fill'] = fill
        if fillOpacity: 
            fillOpacity = fill.lower()
            if fillOpacity != 'none':
                styles['fill-opacity'] = fillOpacity
...
class SVGGeometryPATH(SVGGeometry):
    ...
    def _doCreateGeom(self, instancing):
        ...
        if self._styles['useFill']:
            cu.dimensions = '2D'
            cu.fill_mode = 'BOTH'
            cu.materials.append(self._styles['material'])
        else:
            cu.dimensions = '3D'
